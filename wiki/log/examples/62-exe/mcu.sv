...
//-- de programa (1) o el resultado de la operacion (0)
assign data_lower = (switches_sync[15])? 
           exe_program_counter_reg[15:0] :
           exe_rd_data_reg[15:0];

assign data_upper = (switches_sync[15])? 
           exe_program_counter_reg[31:16] :
           exe_rd_data_reg[31:16];
...
//-- Sacar en el display de 7 segmentos la informadion requerida
...
always_comb begin
    ...
    //-- Salidas por defecto
    mem_status_backwards = pipeline_status::READY;

    if (INIT) begin
        mem_status_backwards = pipeline_status::READY;
    end
    else if (E0) begin
        mem_status_backwards = pipeline_status::STALL;
    end
    else if (E1) begin
        mem_status_backwards = pipeline_status::READY;
    end
